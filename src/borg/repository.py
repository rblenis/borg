...
class Repository:
    ...
    def get_index_transaction_id(self):
        indices = sorted(int(fn[6:]) for fn in os.listdir(self.path) if fn.startswith('index.') and
                         fn[6:].isdigit() and os.stat(os.path.join(self.path, fn)).st_size != 0)
        if indices:
            return indices[-1]
        else:
            return None
            ...
            for tag, key, offset, data in self.io.iter_objects(segment, include_data=True):
                ...
                if tag == TAG_PUT and is_index_object:
                    ...
                    if shadowed_put_exists or delete_is_not_stable:
                        ...
                        # this can happen. In this case it doesn't cause outright corruption, 'just' an index count
                        # mismatch, which will be fixed by borg-check --repair.
                        #
                        # Note that in this check the index state is the proxy for a "most definitely settled"
                        # repository state, ie. the assumption is that *all* operations on segments <= index state
...
class LoggedIO:
    ...
    def _read(self, fd, fmt, header, segment, offset, acceptable_tags, read_data=True):
        ...
        if read_data:
            data = fd.read(length)
            if len(data) != length:
                raise IntegrityError('Segment entry data short read [segment {}, offset {}]: expected {}, got {} bytes'
                    .format(segment, offset, length, len(data)))
            ...
            if key is None and tag in (TAG_PUT, TAG_DELETE):
                ...
                length -= 32
                if len(key) != 32:
                    raise IntegrityError('Segment entry key short read [segment {}, offset {}]: expected {}, got {} '
                        'bytes'.format(segment, offset, 32, len(key)))
            oldpos = fd.tell()
            seeked = fd.seek(length, os.SEEK_CUR) - oldpos
            data = None
            if seeked != length:
                raise IntegrityError('Segment entry data short seek [segment {}, offset {}]: expected {}, got {} bytes'
                    .format(segment, offset, length, seeked))
        if tag not in acceptable_tags:
            raise IntegrityError('Invalid segment entry header, did not get acceptable tag [segment {}, offset {}]'
