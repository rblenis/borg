...
class TestExclusiveLock:
    ...
    def test_race_condition(self, lockpath):
        ...
        def acquire_release_loop(id, timeout, thread_id, lock_owner_counter, exception_counter, print_lock,
                                 last_thread=None):
            print_locked("Thread %2d: Starting acquire_release_loop(id=%s, timeout=%d); lockpath=%s" %
            ...
            while not timer.timed_out():
                cycle += 1
                try:
                    # This timeout is only for not exceeding the given timeout by more than 5%. With sleep<0 it's
                    # constantly polling anyway.
                    with ExclusiveLock(lockpath, id=id, timeout=timeout/20, sleep=-1):
                        lock_owner_count = lock_owner_counter.incr()
                        print_locked("Thread %2d: Acquired the lock. It's my %d. loop cycle. I am the %d. who has "
                                     "the lock concurrently." % (thread_id, cycle, lock_owner_count))
                        time.sleep(0.005)
                        lock_owner_count = lock_owner_counter.decr()
                        print_locked("Thread %2d: Releasing the lock, finishing my %d. loop cycle. Currently, %d "
                                     "colleagues still have the lock." % (thread_id, cycle, lock_owner_count))
                except LockTimeout:
                    print_locked("Thread %2d: Got LockTimeout, finishing my %d. loop cycle." % (thread_id, cycle))
                except:
                    exception_count = exception_counter.incr()
                    e = format_exc()
                    print_locked("Thread %2d: Exception thrown, finishing my %d. loop cycle. It's the %d. exception "
                                 "seen until now: %s" % (thread_id, cycle, exception_count, e))
        ...
        for thread_id in range(RACE_TEST_NUM_THREADS):
            thread = Thread(target=acquire_release_loop, args=(('foo', thread_id, 0), RACE_TEST_DURATION,
                                                               thread_id, lock_owner_counter, exception_counter,
                                                               print_lock, thread))
            thread.start()
        thread.join()  # joining the last thread

        assert lock_owner_counter.maxvalue() > 0, \
            'Never gained the lock? Something went wrong here...'
        assert lock_owner_counter.maxvalue() <= 1, \
            "Maximal number of concurrent lock holders was %d. So exclusivity is broken." % (
                lock_owner_counter.maxvalue())
        assert exception_counter.value() == 0, \
