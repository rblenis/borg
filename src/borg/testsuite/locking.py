...
class TestExclusiveLock:
    ...
    def test_race_condition(self, lockpath):
            ...
            while not timer.timed_out():
                ...
                try:
                    ...
                    with ExclusiveLock(lockpath, id=id, timeout=timeout/20, sleep=-1):
                        ...
                        print_locked("Thread %2d: Acquired the lock. It's my %d. loop cycle. I am the %d. who has the "
                                     "lock concurrently." % (thread_id, cycle, lock_owner_count))
                        time.sleep(0.005)
        ...
        for thread_id in range(RACE_TEST_NUM_THREADS):
            thread = Thread(target=acquire_release_loop, args=(('foo', thread_id, 0), RACE_TEST_DURATION, thread_id,
        ...
        assert lock_owner_counter.maxvalue() <= 1, "Maximal number of concurrent lock holders was %d. So exclusivity " \
            "is broken." % (lock_owner_counter.maxvalue())
        assert exception_counter.value() == 0, "ExclusiveLock threw %d exceptions due to unclean concurrency handling."\
