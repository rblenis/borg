...
from ..helpers import PlaceholderError, replace_placeholders
...
class TestLocationWithoutEnv:
    ...
    def test_format_path(self, monkeypatch):
        monkeypatch.delenv('BORG_REPO', raising=False)
        test_pid = os.getpid()
        assert repr(Location('/some/path::archive{pid}')) == "Location(proto='file', user=None, host=None, port=None," \
            " path='/some/path', archive='archive{}')".format(test_pid)
...
class TestParseTimestamp(BaseTestCase):
    ...
    def test(self):
        self.assert_equal(parse_timestamp('2015-04-19T20:25:00.226410'), datetime(2015, 4, 19, 20, 25, 0, 226410,
            timezone.utc))
...
def test_get_security_dir(monkeypatch):
    ...
    assert get_security_dir(repository_id='1234') == os.path.join(os.path.expanduser('~'), '.config', 'borg',
        'security', '1234')
    monkeypatch.setenv('XDG_CONFIG_HOME', '/var/tmp/.config')
